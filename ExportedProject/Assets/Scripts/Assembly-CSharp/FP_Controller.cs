...
public class FP_Controller : MonoBehaviour
{
	...
	{
		inputModifyFactor = ((inputX == 0f || inputZ == 0f) ? 1f : 0.7071f);
		if (grounded)
		{
			sliding = false;
	...
	private void OnControllerColliderHit(ControllerColliderHit hit)
	{
		...
		{
			float num = 1f;
			attachedRigidbody.velocity = (new Vector3(attachedRigidbody.position.x, 0f, attachedRigidbody.position.z) - new Vector3(myTransform.position.x, 0f, myTransform.position.z)).normalized * num * hit.controller.velocity.magnitude / attachedRigidbody.mass;
		}
	...
	private bool CanStand()
	{
		...
		if (controller.height < defaultHeight - 0.1f)
		{
			return !Physics.SphereCast(controller.bounds.center, controller.radius, Vector3.up, out hitInfo, controller.height / 2f);
		}
		return true;
	}
